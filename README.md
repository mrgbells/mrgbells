<div align="center">

# Hi, I'm Abel Chigombe

**Penetration Tester (PNPT, Security+) · MSc Cybersecurity Candidate @ Northeastern · AI/LLM Security**

Open to cybersecurity internships or co-ops (offensive security, cloud, AI/LLM red teaming, GRC, SOC, threat intelligence)

[Portfolio](https://abelchigombe.netlify.app/) · [LinkedIn](https://www.linkedin.com/in/abelchigombe) · [Medium](https://medium.com/@abelchigombetatenda) · [Email](mailto:chigombe.a@northeastern.edu)

![OSCP](https://img.shields.io/badge/OSCP-In%20Progress-6b7280?style=flat-square)
![PNPT](https://img.shields.io/badge/PNPT-Certified-b91c1c?style=flat-square)
![Security+](https://img.shields.io/badge/CompTIA%20Security%2B-Certified-2563eb?style=flat-square)

</div>

---

## About

I'm an offensive security practitioner with 3+ years of hands-on penetration testing experience across 20+ client engagements at **Baker Tilly Central Africa** (web application, network, and infrastructure VAPT across Southern Africa), and prior experience as a **Cybersecurity Research and Teaching Assistant** at the University of Zimbabwe, where I built Docker-based CTF infrastructure and taught ethical hacking and information systems audit.

I'm now an MSc Cybersecurity student at Northeastern University. Most of my personal project time goes toward the attack surface most pentesters aren't testing yet: AI and LLM systems, alongside cloud security and Active Directory labs.

## What I work on

- **Offensive Security** — network and web application penetration testing, Active Directory exploitation (Kerberoasting, Pass-the-Hash, ADCS abuse), adversary emulation mapped to MITRE ATT&CK
- **AI/LLM Security** — prompt injection, jailbreak testing, RAG attack classification, LLM firewalls, mapped to the OWASP LLM Top 10
- **Cloud Security** — AWS/GCP privilege escalation labs, policy-as-code compliance auditing, IAM and Workload Identity misconfiguration defense
- **Vulnerability Management** — end-to-end VM program design, Tenable Nessus, CVSS-based prioritization, remediation scripting

## Featured projects

| Project | What it does |
|---|---|
| [ai-llm-security-toolkit](https://github.com/mrgbells/ai-llm-security-toolkit) | 5 offensive AI/LLM tools automating prompt injection, jailbreak testing, and Ollama-powered RAG attack classification, mapped to the OWASP LLM Top 10 |
| [llm-firewall-gateway](https://github.com/mrgbells/llm-firewall-gateway) | Stateful AI security gateway: PII redaction, prompt injection defense, and denial-of-wallet protection in front of LLM APIs |
| [Vulnerability-Management-Program-Implementation](https://github.com/mrgbells/Vulnerability-Management-Program-Implementation) | End-to-end vulnerability management program simulation using Tenable Nessus and Azure VMs, 81% total vulnerability reduction |
| [gke-iam-wi-defense-lab](https://github.com/mrgbells/gke-iam-wi-defense-lab) | GKE to IAM privilege escalation lab: Workload Identity misconfiguration, with both a vulnerable and a hardened environment |
| [multicloud-opa-compliance-auditor](https://github.com/mrgbells/multicloud-opa-compliance-auditor) | Policy-as-code compliance auditor blocking unsafe Terraform and ARM changes across AWS, Azure, and GCP using OPA and Conftest |
| [adversarial-ml-ember-lab](https://github.com/mrgbells/adversarial-ml-ember-lab) | Adversarial ML lab: feature-space evasion attacks on EMBER-style malware detectors using ART and SHAP, with adversarial training defenses |
| [threat-hunting-scenario-tor](https://github.com/mrgbells/threat-hunting-scenario-tor) | Threat hunt report detecting unauthorized TOR usage using Microsoft Defender for Endpoint and KQL |
| [kali-vmware-cursor-fix](https://github.com/mrgbells/kali-vmware-cursor-fix) | The 2-line fix for the invisible cursor bug on Kali Linux in VMware, caused by an outdated virtual hardware version |

See [all repositories](https://github.com/mrgbells?tab=repositories) for the full list.

## Writing

- [How I Passed the PNPT on My First Attempt](https://medium.com/@abelchigombetatenda/how-i-passed-the-pnpt-practical-network-penetration-tester-exam-a-compliant-guide-to-success-3407924cef6b)
- [Cracking the Security+ Exam: A Practical Blueprint for Success](https://medium.com/@abelchigombetatenda/cracking-the-security-exam-a-practical-blueprint-for-success-3cba87225646)
- [Critical Infrastructure Risk Analysis: Cyber-Physical Resilience of the T-Mobile Boston Network](https://medium.com/@abelchigombetatenda/critical-infrastructure-risk-analysis-cyber-physical-resilience-of-the-t-mobile-boston-00003aead306)

## Let's connect

I'm looking for a cybersecurity internship or co-op (offensive security, cloud, AI/LLM red teaming). If you're hiring, or want to talk Active Directory attacks, cloud misconfigurations, or LLM red teaming, I'd love to hear from you.

[chigombe.a@northeastern.edu](mailto:chigombe.a@northeastern.edu) · [LinkedIn](https://www.linkedin.com/in/abelchigombe) · [Portfolio](https://abelchigombe.netlify.app/)
